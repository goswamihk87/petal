...
public class CustomEditText extends AppCompatEditText {
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            TypedArray arrEditView = getContext().obtainStyledAttributes(attrs, R.styleable.CustomEditText);
            String fontName = arrEditView.getString(R.styleable.CustomEditText_fontName);
            if (fontName != null && !isInEditMode()) {
                Typeface typeface = mFontCache.getTypeface(fontName);
                setTypeface(typeface);
            }

            boolean allowFirstSpace = arrEditView.getBoolean(R.styleable.CustomEditText_allowFirstSpace, false);
            if (!allowFirstSpace) {
                filterFirstChars(' ');
            }

            int filterType = arrEditView.getInteger(R.styleable.CustomEditText_filterType, 0);
